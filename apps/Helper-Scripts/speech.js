...
window.loadVoices = function() {
    ...
    if (window.voiceList.length > 0) {
        ...
        if (savedName) {
            const voice = window.voiceList.find(v => v.name === savedName);
            if (voice && (voice.lang.startsWith('id') || voice.lang.includes('id-ID'))) {
        ...
        if (!window.preferredVoice) {
            const indonesianVoices = window.voiceList.filter(v => v.lang.startsWith('id') || v.lang.includes('id-ID'));
        ...
        // We force Indonesian lang, so we do not fall back to English voices
    }
};
...
window.speakText = function(text, onEndCallback) {
    ...
    if (window.preferredVoice && (window.preferredVoice.lang.startsWith('id') || window.preferredVoice.lang.includes('id-ID'))) {
